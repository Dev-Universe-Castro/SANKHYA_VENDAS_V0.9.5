...
export async function consultarAtividades(codLead: string, idEmpresa: number, ativo: string = 'S'): Promise<any[]> {
  ...
  if (result.length > 0) {
    console.log('📝 [Oracle] Primeira atividade:', JSON.stringify(result[0], null, 2));
  }
  ...
  const atividades = (result || []).map((row: any) => {
    ...
    // Tenta separar a descrição em título e detalhe usando '|' como delimitador.
    const descricaoCompleta = row.DESCRICAO || '';
    const partesDescricao = descricaoCompleta.split('|');
    const titulo = partesDescricao.length > 1 ? partesDescricao[0].trim() : descricaoCompleta.trim();
    const detalheDescricao = partesDescricao.length > 1 ? partesDescricao.slice(1).join('|').trim() : '';
    ...
    const atividade = {
      ...
      ORDEM: row.ORDEM,
      ATIVO: row.ATIVO || 'S',
      STATUS: dadosComplementares.STATUS || 'AGUARDANDO'
    };
...
export async function criarAtividade(atividade: Partial<LeadAtividade>, idEmpresa: number): Promise<LeadAtividade> {
  ...
  try {
    ...
    // Montar a string de descrição combinando Título e Descrição detalhada, se ambos existirem
    let descricaoCompleta = '';
    if (atividade.DESCRICAO) {
        descricaoCompleta = `${atividade.DESCRICAO}|${atividade.DADOS_COMPLEMENTARES ? JSON.stringify(atividade.DADOS_COMPLEMENTARES) : ''}`;
    } else if (atividade.DADOS_COMPLEMENTARES) {
        descricaoCompleta = `${JSON.stringify(atividade.DADOS_COMPLEMENTARES)}`;
    }
    ...
    const sql = `
      INSERT INTO AD_ADLEADSATIVIDADES (
        CODLEAD, ID_EMPRESA, TIPO, DESCRICAO, DATA_HORA, DATA_INICIO, DATA_FIM,
    ...
    await oracleService.executeQuery(sql, {
      ...
      idEmpresa,
      tipo: atividade.TIPO,
      descricao: atividade.DESCRICAO || null, // Alterado para usar DESCRICAO se disponível
      dataHora: formatarDataParaOracle(new Date().toISOString()), // Data e hora atual como padrão
    ...
    // A atividade recém-criada deve ser a primeira na lista devido à ordenação por ORDEM DESC
    return atividades[0];
